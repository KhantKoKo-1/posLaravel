                                            ...
                                            <tr class="even pointer">
                                                ...
                                                <td class=" ">
                                                    <span class="badge badge-primary"
                                                        style="display:{{ $account->status === 0 ? 'inline' : 'none' }}">Enable</span>
                                                    <span class="badge badge-secondary"
                                                        style="display:{{ $account->status === 1 ? 'inline' : 'none' }}">Disable</span>
                                                </td>
                                                <td class="last">
                                                    <div class="row">
                                                        <div class='col-2 col-md-2'>
                                                            <a href="{{ url('sg-backend/account/edit/' . $accountType . '/account_info/' . $account->id) }}"
                                                                class="btn btn-info btn-xs"><i class="fa fa-pencil"></i>
                                                                Edit</a>
                                                        </div>
                                                        <div class='col-3 col-md-3'>
                                                            <a href="{{ url('sg-backend/account/edit/' . $accountType . '/password/' . $account->id) }}"
                                                                class="btn btn-info btn-xs"><i class="fa fa-pencil"></i>
                                                                Change Password </a>
                                                        </div>
                                                        <div class='col-3 col-md-3'>
                                                            <form id="accountDelete"
                                                                action="{{ url('sg-backend/account/delete') }}"
                                                                method="POST">
                                                                @csrf
                                                                <button type="button" class="btn btn-danger btn-xs"
                                                                    onclick="confirmDelete('accountDelete',{{ $account->id }})">
