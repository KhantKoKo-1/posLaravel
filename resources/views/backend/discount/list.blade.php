                            ...
                            <tr class="even pointer" >
                              ...
                              @if($discount->getPromotion != null)
                                  ...
                                  @foreach ($discount->getPromotion as $item)
                                    ...
                                    @php
                                        $namesArray[] = $item->getPromotionByItem->name; // Push each name into the array
                                    @endphp
                                    @endif   
                                  @endforeach
                              ...
                              <td class=" ">
                                  <span class="badge badge-primary" style="display: {{ $discount->status == 0 ? 'block' : 'none' }}">Enable</span>  
                                  <span class="badge badge-secondary" style="display: {{ $discount->status != 0 ? 'block' : 'none' }}">Disable</span>
                              </td>
                              <td class="last">
                              <div class="row">
                                <div class="col-6 col-md-5">
                                  <a href="{{ url('sg-backend/discount/edit/' . $discount->id) }}" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> Edit </a>
                                </div>  
                                <div class="col-6 col-md-6 mr-20">
                                    <form id="discountDelete" action="{{ url('sg-backend/discount/delete') }}" method="POST">
                                        @csrf
                                        <button type="button" class="btn btn-danger btn-xs" style="" onclick="confirmDelete('discountDelete',{{  $discount->id  }})">
                                            <i class="fa fa-trash-o"></i> Delete
                                        </button>
                                        <input name="id" id="delete_id" type="hidden" value="" />
                                    </form>
                                </div>
                                </div>
                              </td>
