...
app.controller("myCtrl", function ($scope, $http) {
    ...
    $scope.init = function (orderId) {
        ...
        $http({
            method: "POST",
            url: base_url + "api/order_edit",
            data: { order_id: orderId },
        }).then(
            function (response) {
                if (response.status == 200) {
                    $scope.itemDatas = response.data;
                ...
                $scope.itemDatas = $scope.itemDatas.map(function (item) {
                    if (item_id === item.id) {
                        ...
                        item.discount_amount =
                            $scope.totalDiscount * item.quantity;
                    }
                    return item;
                });
    ...
    $scope.makeOrder = function () {
        ...
        const url = base_url + "api/make_order";
        $http
            .post(url, data)
            .then(function (response) {
                $scope.checkResult = response.data["status"];
                if ($scope.checkResult === "success") {
