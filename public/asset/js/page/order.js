        ...
        .then(function(response) {
            ...
            $scope.itemDatas = $scope.itemDatas.map(function(item) {
                if (item_id === item.id) {
                    ...
                    item.discount_amount = $scope.subDiscount[item_id] * item.quantity;
                }
                return item;
            });
            ...
            if (!$scope.sameItem) {
                ...
                $scope.subDiscount[id] = parseInt(response.data.data[0].discount_amount);
            }
   ...
   $scope.itemQuantity = function(type, itemId) {
    $scope.itemDatas = $scope.itemDatas.map(function(item) {
        if (itemId == item.id) {
            ...
            item.discount_amount = $scope.subDiscount[itemId] * item.quantity;
        }
        return item;
    });
    $scope.calculationSubTotable();
