...
app.controller('myCtrl', function($scope,$http) {
  ...
  $scope.cashPayment = function(money){
    let payment = { cash: money,origin_amount: money,index: $scope.cashAmounts.length + 1,quantity : 1 };
    ...
    if($scope.paymentQty != '' && $scope.paymentQty != 1){
        ...
        payment = { cash: parseInt(amount),origin_amount: money, index: $scope.cashAmounts.length + 1,quantity : $scope.paymentQty };
  ...
  $scope.cashMoneyClick = function(index){
    ...
    indexExists ? $scope.selectIndex.splice($scope.selectIndex.indexOf(index), 1) : $scope.selectIndex.push(index);
  }

  $scope.void = function() {
    let total_amount = $scope.totalAmount;
    $scope.cashAmounts = $scope.cashAmounts.filter(item => !$scope.selectIndex.includes(item.index));
    $scope.payMoney  = 0;
